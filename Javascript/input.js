...
const spriteSelection = {
    ...
    startGame() {
        ...
        } catch (e) { console.error('music manager init failed', e); }
...
function lorMystery_player1_ability_2() {
    // Play laser sound
    try {
        if (typeof musicManager !== 'undefined' && typeof musicManager.playLaserSound === 'function') {
...
function lorMystery_player1_ability_3() {
    ...
    if (typeof musicManager !== 'undefined' && typeof musicManager.playExplosionSound === 'function') {
...
function resetRound() {
    ...
    // Remove HUD elements from previous round
    ...
    // Remove ability UI if it exists
    const p1AbilityUI = document.getElementById('player1-abilities');
    if (p1AbilityUI) {
        p1AbilityUI.remove();
    }
    const p2AbilityUI = document.getElementById('player2-abilities');
    if (p2AbilityUI) {
        p2AbilityUI.remove();
    }
    
    // Reset ability cooldowns properly
    if (typeof abilityCooldowns !== 'undefined') {
        abilityCooldowns.player1 = {
            PA: 0,
            ability1: 0,
            ability2: 0,
            ability3: 0,
    ...
    // Clear projectiles properly
    if (typeof projectiles !== 'undefined') {
        // Remove all projectile elements from DOM first
        for (let proj of projectiles.list) {
    ...
    // Also clean up any remaining stray divs that might be projectiles
    const gameContainer = document.getElementById('gameContainer');
    const allChildren = Array.from(gameContainer.children);
    for (let child of allChildren) {
        // Keep only the essential containers
        if (child.id !== 'terrain' && child.id !== 'player1' && child.id !== 'player2' && 
            !child.id.includes('Hud') && !child.id.includes('abilities') && 
            !child.id.includes('Overlay')) {
            // This is likely a projectile, remove it
            try {
                child.remove();
            } catch (e) {}
        }
    ...
    // Reset players (clear styles but keep elements)
    const player1Element = document.getElementById('player1');
    const player2Element = document.getElementById('player2');
    if (player1Element) {
        player1Element.style.cssText = '';
    }
    if (player2Element) {
        player2Element.style.cssText = '';
    }
